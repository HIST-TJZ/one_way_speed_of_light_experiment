...
For a photon path of length R, the number of grains crossed: N_cells = R/λ₀. Fluctuation per cell: δc/c = 1/√N_grain. Accumulated fluctuation: σ_z = (v/c) · √(Rλ₀) · δc/c = (v/c) · √(Rλ₀/N_grain).

For detectability with the optimal setup (σ_z > 0.03 nm): √(λ₀/N_grain) > 3×10⁻⁸. If N_grain ~ 1: λ₀ > 10⁻¹⁵ m (nuclear scale). If N_grain ~ 10⁶: λ₀ > 10⁻⁹ m (nanometer scale).
...
Spacetime "atoms" (10⁻³⁵ m) Collective modes (μm–m)                 Trajectory distortions (AFM)
                                      ↑
...
**Setup**: Fill the cylinder with a fluid near its critical point (e.g., CO₂ at 31°C / 73 atm, or SF₆ at 45°C / 38 atm). Near the critical point, density fluctuations diverge — the correlation length ξ expands from molecular scale to the μm–mm range, producing visible critical opalescence.
...
**Standard prediction**: Trajectory roughness is fully determined by Ornstein-Zernike scattering theory + dynamic scaling, with known and thoroughly verified critical exponents.

**Non-standard signal**: If the observed scaling exponents deviate from OZ theory → an additional fluctuation source exists — not density fluctuations, but spacetime fluctuations — which couples to density fluctuations and is amplified by criticality.
...
**Standard prediction**: All three trajectories are **exactly identical** — Maxwell's equations care only about the macroscopic refractive index n, not chemical composition.

**Non-standard signal**: Any statistically significant difference between any two trajectories → **the speed of light is not solely a function of n** → chemical composition affects light speed through a non-electromagnetic channel (spacetime-medium coupling). A signal that "should not exist" theoretically.
...
Standard prediction: Trajectory exhibits sinusoidal modulation with amplitude precisely determined by dn/dρ, and phase fully synchronized with the acoustic density wave.
```

**Non-standard signal**: If the trajectory modulation exhibits a **phase lag** relative to the acoustic density wave → the spacetime medium has a finite relaxation time τ. A phase lag cannot be explained by any Maxwell-equation adjustment — it is a binary existence proof.
...
```
Electric field: Parallel plate electrodes, ~10⁶ V/m
Magnetic field: Superconducting magnet or Halbach array, ~1–10 T
```

**Standard prediction**: DC Kerr and Cotton-Mouton effects on n are at the 10⁻¹⁰–10⁻¹² level — at experimental precision, **the trajectory should be completely unaffected. The standard-predicted signal is zero.**

**Non-standard signal**: Any trajectory change → c depends on the electromagnetic field → vacuum has nonlinear electromagnetic response beyond QED vacuum polarization → light speed is field-controllable.

---

### Variant 5: Sharp Interface — The Refractive-Index Jump Fingerprint

**Setup**: Two immiscible transparent liquids (e.g., water and silicone oil), or an air-liquid interface. The laser plane crosses the interface during rotation.

```
Air/water interface (n jumps from 1.000 to 1.333):
Standard prediction: A clean, sharp vertical jump between the two segments
Δz' = vR(n₂−n₁)/c ≈ 33 nm (for R=3 m, v=10 m/s)
```

**Non-standard signals**: (1) Transition width >> physical interface width (spacetime relaxation); (2) Small oscillations after the jump (spacetime "ringing"); (3) Trajectory begins deviating before reaching the interface — "premonition" (non-local precursor); (4) Entry ≠ exit asymmetry (emergent hysteresis).

---

### Variant 6: Temperature Gradient — "Thermal Response" of the Spacetime Medium

**Setup**: Heat the cylinder bottom and cool the top to establish a vertical temperature gradient. Density varies with temperature → refractive index varies continuously.

```
T(z') from 20°C (top) to 5°C (bottom)
Standard prediction: Trajectory curvature fully determined by ∫₀ᴿ n(T(z'), r) dr
```

**Non-standard signal**: Trajectory curvature deviates from n(T) prediction. More importantly — **if temperature directly affects the "stiffness" of the spacetime medium**, rather than merely changing the physical medium's density.

---

### Variant 7: Flowing Medium — An Emergent Version of Fizeau Drag

**Setup**: Liquid flows radially within the cylinder (pump-driven). Photons traverse the flowing medium during radial flight.

```
Co-flow: water flows outward (parallel to photon direction)
Counter-flow: water flows inward (anti-parallel)
```

**Standard prediction** (Fresnel-Fizeau drag, verified since 1851): Effective light speed = c/n ± v_flow(1−1/n²).

**Non-standard signal**: If the drag coefficient deviates from f = 1−1/n² → the medium drags not merely the electromagnetic phase velocity, but partially drags the spacetime medium itself.

---

### Variant 8: Multi-Wavelength Dispersion — "Band Structure" of the Spacetime Medium

**Setup**: Use two lasers of different wavelengths. Record two trajectories on the same cylinder.

```
Laser 1: λ₁ = 633 nm (He-Ne red)
Laser 2: λ₂ = 405 nm (semiconductor violet)
Standard prediction: Δ(Δx) = (vR/c)·[n(λ₂)−n(λ₁)], fully determined by the Sellmeier dispersion formula.
```

**Non-standard signal**: If Δ(Δx) deviates from the Sellmeier formula → the dispersion of the spacetime medium differs from the EM dispersion of the physical medium. If anomalous dispersion peaks appear → the spacetime medium has a "band structure."

---

### Variant 9: Rotating the Entire Apparatus — Searching for a Preferred Reference Frame

**Setup**: Rotate the entire apparatus (laser + cylinder + medium) as a rigid body about a vertical axis at angular velocity Ω.

```
Standard prediction (Sagnac effect): Extra deflection ≈ 2ΩR/c. For R=3 m, Ω=10 rad/s → ~2×10⁻⁷ rad → far below resolution. Standard prediction: no observable effect.
```

**Non-standard signal**: If emergent spacetime has a preferred reference frame, the one-way speed of light measured in different directions will differ. The trajectory will exhibit a θ-dependent periodic modulation with period 2π — the single-pass version of the Michelson-Morley experiment, waiting 140 years.

---

### Variant 10: Time-Varying Medium — "Memory Effect" of the Spacetime Medium

**Setup**: Actively change the medium density during the brief interval (~6 ms) in which the laser sweeps the cylinder.

```
t = 0: State A; t = 3 ms: Rapid switch to state B; t = 6 ms: One revolution complete
Standard prediction: Continuous transition at the corresponding angle, slope determined by dn/dt.
```

**Non-standard signal**: If the trajectory begins deviating **before** the density change → the spacetime medium has "premonition" of an impending change. Or the opposite — **after** the density change the trajectory slowly catches up → the spacetime medium has inertia/memory.

---

### Variant 11: Static Gravitational Gradient — Does Gravity Influence c Beyond GR?

**Setup**: Place a high-density mass (lead or tungsten block, 10³–10⁴ kg) fixed beside the cylinder. Photons in different θ directions have different closest-approach distances to the mass.
...
**Standard prediction (GR)**: Shapiro delay differential ~10⁻³¹ s, corresponding to trajectory deviation ~10⁻³⁰ m. **At experimental precision, strictly zero.**

**Non-standard signal**: If matter influences light speed by directly modifying the "elastic modulus" of the spacetime medium (rather than curving spacetime), the effect may far exceed the GR Shapiro delay. Trajectory exhibits a θ-dependent modulation with minimum in the direction of the mass.

---

### Variant 12: Accelerating Cylinder — Single-Pass Test of the Equivalence Principle

**Setup**: The cylinder falls with constant acceleration a (free-fall or mechanically driven). Velocity varies with time: v(t) = v₀ + at. Trajectory becomes a parabola:

```
z'(θ) = v₀(θ/ω + R/c) + (a/2)(θ/ω + R/c)²
```

**Standard prediction**: A precise parabola, curvature fully determined by independently measured a and v₀. The equivalence principle (a cornerstone of GR) asserts this parabola should hold perfectly.

**Non-standard signal**: Deviation from the standard parabola → the equivalence principle is violated at the single-pass light-propagation level. **Never tested experimentally** — all existing equivalence-principle tests use round-trip light signals or non-optical probes.

---

### Variant 13: Polarization Dependence — Does the Spacetime Medium Have an "Optical Axis"?

**Setup**: Same cylinder, multiple runs — switching the laser polarization state each time (horizontal linear, vertical linear, left/right circular).

**Standard prediction (in vacuum)**: All polarization states yield exactly the same trajectory. Vacuum is perfectly isotropic in Maxwell's equations.

**Non-standard signal**: Statistically significant differences between polarization states → the spacetime medium itself has intrinsic anisotropy — a "preferred direction." This is predicted by Lorentz-violating Standard Model Extensions (SME), but **never searched for in a single-pass experiment.** Cost: a set of polarizers/wave plates — virtually zero.

---

### Variant 14: Rotating Medium (Vortex) — Angular Momentum Coupling

**Setup**: Liquid rotates within the cylinder at angular velocity Ω_fluid about the axis, forming a stable vortex. Photons traverse different angular-velocity layers during radial flight.
...
**Standard prediction**: For radially propagating photons, the radial projection of rotational Fresnel-Fizeau drag is zero. Standard EM predicts a trajectory **exactly identical** to the stationary case.

**Non-standard signal**: The angular momentum of the medium directly "drags" the spacetime medium — analogous to GR frame-dragging (Lense-Thirring effect) but at the desktop scale. Complements Variant 7 (linear momentum coupling).
...
```
Run 1: E=0, M=0 (baseline); Run 2: E>0, M=0 (EM only)
Run 3: E=0, M>0 (gravity only); Run 4: E>0, M>0 (combined)
Compare: effect of Run 4 ?=? effect of Run 2 + effect of Run 3
```

**Standard prediction**: Linear superposition. Both individual effects ≈ zero → superposition ≈ zero.

**Non-standard signal**: Combined effect > linear superposition → emergent coupling is nonlinear — the EM field changes the responsiveness of the spacetime medium to gravity. This is the only variant testing "cross-coupling."

---

### Variant 16: Cylinder Self-Rotation — Is Vacuum a Medium?

**Setup**: The cylinder rotates at angular velocity Ω_cyl about its own axis. The interior is **vacuum** (or stationary air). The laser rotates at ω; both share the same axis.
...
**Standard prediction**: In the lab frame, the photon travels in a purely radial straight line — flight time R/c is entirely independent of cylinder rotation. The interior is vacuum; there is no medium to be "dragged." **The standard-predicted signal is strictly zero.**
